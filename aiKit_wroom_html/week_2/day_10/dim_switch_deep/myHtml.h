...
<body>
    <div class='container'>
        <div class='header'>
            <h1>🌈 ESP32 마법 상자</h1>
            <div class='subtitle'>LED와 스위치를 만져보세요!</div>
        </div>
        ...
        <div class='switch-info'>
            ...
            <div class='switch-status'>
                <span class='emoji'>{{SWITCH_ICON}}</span>
                <span id='switch-state'>{{SWITCH_STATE}}</span>
            </div>
            <p>GPIO 32번에 스위치가 연결되어 있어요!</p>
        </div>
    ...
    <script>
        ...
        function updateSwitchStatus() {
            fetch('/switch')
                ...
                .then(state => {
                    const switchStateDiv = document.getElementById('switch-state');
                    if (state === 'on') {
                        switchStateDiv.textContent = '켜짐';
                        switchStateDiv.className = 'switch-on';
                    } else {
                        switchStateDiv.textContent = '꺼짐';
                        switchStateDiv.className = 'switch-off';
                    }
                });
        }
