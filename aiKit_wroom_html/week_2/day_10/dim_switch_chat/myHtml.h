  ...
  <div class="wrap">
    <div class="card">
      ...
      <div class="grid">
        ...
        <!-- Switch 상태 -->
        <div class="tile" id="switchBox" style="grid-column: span 3; background:#fff3e0;">
          <div class="name">🔘 스위치 (D32)</div>
          <div style="font-size:22px; margin:8px 0;">
            상태: <span id="switchState">알 수 없음</span>
          </div>
        </div>
      </div>
...
<script>
  ...
  async function updateSwitch(){
    try{
      ...
      document.getElementById('switchState').textContent = txt;
    } catch(e){
      document.getElementById('switchState').textContent = "연결 오류";
    }
  }
  setInterval(updateSwitch, 1000); // 1초마다 갱신
  updateSwitch();
</script>
</body>
</html>
